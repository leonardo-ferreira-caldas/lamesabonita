$(document).ready(function() {
    ...
    $("#salvar_menu").submit(function(evt) {
        var empty = true;
        var size  = false;
        ...
        if ($(".checkbox_tipo_refeicao input:checked").size() == 0) {
            Utils.alert.error("Validação", "Selecione pelo menos um tipo de refeição para o seu menu.");
            evt.preventDefault();
            return false;
        }

        if ($(".checkbox_tipo_culinaria input:checked").size() == 0) {
            Utils.alert.error("Validação", "Selecione pelo menos um tipo de culinária para o seu menu.");
            evt.preventDefault();
            return false;
        }

        //if (empty && $(".uploaded-picture-menu").size() == 0) {
        //    Utils.alert.error("Validação", "Selecione pelo menos uma foto para o menu.");
        //    evt.preventDefault();
        //    return false;
        //}
    });
