...
var selectMenuCursoPicture = function() {
	...
	$(this).parent().parent().addClass("menu-picture-selected");
};
...
$("#add_price_menu").click(function(evt) {
	evt.preventDefault();

	var template = $('.prices-template').clone();
	template.removeClass('prices-template');
	template.find('input').maskMoney({thousands:'', decimal:'.'});
	template.find('input, select').val('').attr('required', 'required');

	$("#prices_rules").append(template);
    $(".preco_fallback").hide();
...
$(".city-search").focus(function() {
    $("#city-search-label").hide();
});

$(".city-search").blur(function() {
    if ($(this).val().toString().length == 0) {
        $("#city-search-label").show();
    }
});

$('.menu-form-pictures input').change(selectMenuCursoPicture);
...
var getFilters = function() {
	...
	var hiddens = $('.menu_filters_hidden').serializeArray();

	for (var idx in hiddens) {
		filters[hiddens[idx].name] = hiddens[idx].value;
	}

	$(".menu-filters input[type=checkbox]:checked").each(function() {
		filters[$(this).attr("name")].push($(this).val());
	});

	var prices = $("#range_money").data("ionRangeSlider");
	filters['precos'] = [prices.old_from, prices.old_to];
	filters['pagina'] = 1
...
var search = function(filters) {
	...
	$(".overlay_menu_loading").show();

	$.ajax({
		url: '/nossos-menus-cursos',
		data: filters,
		dataType: 'text',
		type: 'GET',
		success: function(response) {
			$('.menu-list-content-wrapper').html(response);
			$('html, body').animate({scrollTop: 150}, 'slow', function() {
				$(".overlay_menu_loading").hide();	
			});
		}
	});
...
$('.menu-filters input[type=checkbox]').click(function() {
	search(getFilters());
});

if ($("#range_money").size() > 0) {
...
$("body").delegate('.custom-pagination a', 'click', function(evt) {

	evt.preventDefault();

	var active = $('.custom-pagination a.active').data('pagina');

	if (active == $(this).data('pagina')) {
		return;
	}
...
$(".menu-sf-select select").change(function() {
	$('.menu-sf-select span').text($(this).val());
});

$("#city_chef").autocomplete({
    serviceUrl: '/buscar-cidades',
    paramName: 'cidade',
    ...
    transformResult: function(response) {
        ...
        if (response.length == 1 && response[0] == '%s') {
            return {suggestions: []};
        }

        return {
            suggestions: response.map(function(obj) {
                return obj.nome_cidade + ", " + obj.fk_estado;
            })
        };
    }
});
