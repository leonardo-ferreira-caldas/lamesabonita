...
<div class="container inner-pages mb-nomargin mb-nopadding mb-nowidth">
    ...
    <div class="col_three_fourth col_last backgroundWhite mb-noborder mb-leftpadding-xsm mb-rightpadding-xsm mb-nobottomargin">
        <div class="fancy-title title-bottom-border">
            <h2>{{ $modo == 'criar' ? 'Nova' : 'Editar' }} Conta Bancária</h2>
        </div>

        @if($modo == 'criar')
            <form method="POST" id="salvar-conta-bancaria" class='use-ajax' action="{{ route('chef.dados_bancarios.novo_salvar') }}">
        @else
            <form method="POST" id="salvar-conta-bancaria" class='use-ajax' action="{{ route('chef.dados_bancarios.editar_salvar', ['id' => $conta_bancaria->id_conta_bancaria]) }}">
        @endif
            ...
            <input type="hidden" name="id_conta_bancaria" value="{{ $conta_bancaria->id_conta_bancaria or '' }}">
            ...
            <div class='col_full'>
                ...
                <select required type="text" id="fk_banco" name="fk_banco" class="sm-form-control required" aria-required="true">
                    ...
                    @foreach ($bancos as $banco)
                        <option
                            {{ $modo == 'editar' && $conta_bancaria->fk_banco == $banco->id_banco ? "selected" : "" }}
            ...
            <div class='col_half'>
                <div class='col_two_third nobottommargin'>
                    ...
                    <input required type="text" value="{{ $conta_bancaria->banco_agencia or '' }}" name="banco_agencia" id="banco_agencia" class="sm-form-control required">
                </div>
                <div class='col_one_third col_last nobottommargin'>
                    <label for="banco_agencia_digito">Digito</label>
                    <input type="text" value="{{ $conta_bancaria->banco_agencia_digito or '' }}" name="banco_agencia_digito" id="banco_agencia_digito" class="sm-form-control required">
                </div>
            </div>
            <div class='col_half col_last'>
                <div class='col_two_third nobottommargin'>
                    <label for="banco_conta">Conta Corrente <small>(obrigatório)</small></label>
                    <input required type="text" value="{{ $conta_bancaria->banco_conta or '' }}" name="banco_conta" id="banco_conta" class="sm-form-control required">
                </div>
                <div class='col_one_third col_last nobottommargin'>
                    <label for="banco_agencia_digito">Digito</label>
                    <input required type="text" value="{{ $conta_bancaria->banco_conta_digito or '' }}" name="banco_conta_digito" id="banco_conta_digito" class="sm-form-control required">
                </div>
            </div>

            <div class='col_full'>
                <label for="banco_proprietario_conta">Proprietário da Conta <small>(obrigatório)</small></label>
                <input required type="text" value="{{ $conta_bancaria->banco_proprietario_conta or '' }}" name="banco_proprietario_conta" id="banco_proprietario_conta" class="sm-form-control required">
            </div>
        ...
        $(document).ready(function() {
            $("form.use-ajax").ajaxForm({
                ...
                afterSuccess: function() {
                    @if($modo == 'criar')
                        $("form#salvar-conta-bancaria").find("input, select").val('');
                    @endif
                }
            });
        });
